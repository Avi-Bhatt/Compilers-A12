...
* Author: Avinish Bhattarai,Nikitaben Chaudhari
...
niv_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern niv_int line; /* Source code line numbers - defined in scanner.c */
 ...
 * -------------------------------------------------------------
 */
niv_void printScannerError(niv_string fmt, ...);
niv_void displayScanner(BufferPointer ptrBuffer);
niv_long getScannerFilesize(niv_string fname);
